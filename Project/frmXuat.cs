...
namespace nhom8
{
    public partial class frmXuat : Form
    {
        ...
        public frmXuat(string viTri, string tenNV)
        {
            InitializeComponent();
            this.viTri = viTri;
            this.tenNV = tenNV;
        ...
        private void LoadData()
        {
            ...
            {
                con.Open();
                string sQuery = "select * from Xuat"; // Giữ nguyên hoặc thay đổi truy vấn nếu cần
                SqlDataAdapter adapter = new SqlDataAdapter(sQuery, con);

                DataSet ds = new DataSet();
                adapter.Fill(ds, "Xuat");

                dataGridView1.DataSource = ds.Tables["Xuat"];
        ...
        {
            tbTimKiem.Text = "Nhập mã hoá đơn xuất cần tìm";
        ...
        {
            if (tbTimKiem.Text == "Nhập mã hoá đơn xuất cần tìm")
            {
                tbTimKiem.Text = "";
        ...
        private void btTim_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(keyword) || keyword == "Nhập mã hoá đơn xuất cần tìm")
            {
                MessageBox.Show("Vui lòng nhập mã hoá đơn xuất cần tìm!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            {
                con.Open();
                string sQuery = "select * from Xuat"; // Giữ nguyên truy vấn hoặc thay đổi nếu cần
                ...
                DataTable dt = ds.Tables["Xuat"];
                DataView dv = new DataView(dt);

                // Điều kiện tìm kiếm theo Mã Dự Nợ (MaDNo)
                dv.RowFilter = string.Format("MaHDX LIKE '%{0}%'", keyword);
        ...
        private void frmXuat_FormClosing(object sender, FormClosingEventArgs e)
        {
