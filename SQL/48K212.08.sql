...
select dbo.TaoMa('Xuat_chitiet')-- Tên bảng vào đấy <-
--Module2 CapNhatTTNV---
select * from Xuat_chitiet
go
...
BEGIN
    ...
    -- Lấy MaSP, GiaNhapMoi, và SLNH từ bảng inserted
...
-- Thêm dữ liệu cho sản phẩm đã tồn tại
INSERT INTO Nhap_ChiTiet (MaHDN, MaSP, SLNH, ThanhTienNH, GiaNhap)
...
begin
    ...
    -- Thêm hóa đơn nhập
    set @mahdn = dbo.Taoma('Nhap'); -- Giả sử có hàm tạo mã cho hóa đơn nhập
    set @ngaynh = getdate();
    declare @hgd date
    set @hgd = dateadd(month, 6, @ngaynh)
...
select * from HDNoNPP
select * from nhanvien
-- Tạo procedure kiểm tra và thêm hóa đơn nhập mới
---Module 10 Thêm Hóa đon xuất
go
create or alter proc ThemHDX(@sdt nvarchar(10),@Manv nvarchar(10),@ten nvarchar(50), @diachi nvarchar(50))
as
begin
	declare @NgayXH date,
			@MaHDX nvarchar(10),
...
BEGIN
    ...
    BEGIN
        ...
        RETURN;  -- Dừng thực hiện nếu sản phẩm không tồn tại
    END
